...
const Sidebar = function Sidebar() {
  const inClick = () => {
    alert('In')
    const btn: HTMLElement = document.getElementById('in-btn')
    btn.style.display = 'none'
    const box: HTMLElement = document.getElementById('out-btn')
    box.style.display = 'block'
  }
  const outClick = () => {
    alert('Out')
    const btn: HTMLElement = document.getElementById('out-btn')
    btn.style.display = 'none'
    const box: HTMLElement = document.getElementById('in-btn')
    box.style.display = 'block'
  }
  const restClick = () => {
    alert('Rest')
    const btn: HTMLElement = document.getElementById('rest-btn')
    btn.style.display = 'none'
    const box: HTMLElement = document.getElementById('back-btn')
    box.style.display = 'block'
  }
  const backClick = () => {
    alert('Back')
    const btn: HTMLElement = document.getElementById('back-btn')
    btn.style.display = 'none'
    const box: HTMLElement = document.getElementById('rest-btn')
    box.style.display = 'block'
  }
  return (
    <>
      
      <div className="h-full tablet:hidden bg-[#004B8F] text-[#fff] relative">
        <div className="flex justify-center py-2">
          <Image src={companyLogo} alt="Company's Logo" />
        </div>
        <div className="flex justify-center py-2">
          <Image src={avatar} alt="Avatar" width={72} height={72} className="rounded-[50%]" />
        </div>
        ...
        <div className="bottom-4 mt-[335px]">
